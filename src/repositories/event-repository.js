...
class EventRepository {
    ...
    async getEventsByFilters(name, category, startDate, tag, limit, offset) {
        ...
        if (category) {
            ...
            const { rows: categoryRows } = await this.client.query(categoryIdQuery);
        ...
        if (tag) {
            ...
            const { rows: tagRows } = await this.client.query(tagIdQuery);
        ...
        try {
            const { rows } = await this.client.query(sqlQuery);
            return rows;
        } catch (error) {
    ...
    async getEventById(id) {
        ...
        try {
            const { rows } = await this.client.query(sqlQuery, [id]);
            return rows[0];
        } catch (error) {
            console.error("Error al ejecutar la consulta SQL:", error);
    ...
    async patchEnrollment(rating, description, attended, observation, id_event, id_user) {
        const existe = await this.client.query(
            `SELECT ee.id, e.start_date FROM event_enrollments ee INNER JOIN events e ON ee.id_event = e.id_event WHERE ee.id_event = $1 AND ee.id_user = $2`,
            [id_event, id_user]
        );
    ...
    async createEvent(name, description, start_date, end_date, location, id_event_category, online, image_url, id_user) {
    ...
    async getEventCategory() {
        let sqlQuery = "SELECT * FROM categories ORDER BY name ASC";
        ...
        try {
            ...
            console.error("Error al ejecutar la consulta SQL:", error);
            throw new Error("Error al obtener las categorías de eventos");
        }
    }

    async getEventLocation() {
        let sqlQuery = "SELECT * FROM locations ORDER BY name ASC";
        ...
        try {
            ...
            console.error("Error al ejecutar la consulta SQL:", error);
            throw new Error("Error al obtener las ubicaciones de eventos");
        }
    }
    ...
    async createEventLocation(name, full_address, latitude, longitude, max_capacity, id_location) {
        const query = `INSERT INTO event_locations(name, full_address, latitude, longitude, max_capacity, id_location) VALUES ('${name}', '${full_address}', ${latitude}, ${longitude}, ${max_capacity}, ${id_location})`;
        return await this.client.query(query);
    }
